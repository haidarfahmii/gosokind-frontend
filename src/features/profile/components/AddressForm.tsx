...
export default function AddressForm({ initialData, onClose, onSuccess }: AddressFormProps) {
    ...
    const hasLocationError = (formik.touched.latitude && formik.errors.latitude) ||
            ...
            <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl relative animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={formik.handleSubmit} className="space-y-5">
                    <div className="space-y-2">
                        ...
                        />
                        {formik.touched.label && formik.errors.label && <p className="text-xs text-red-500">{formik.errors.label as string}</p>}
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        />
                        {formik.touched.fullAddress && formik.errors.fullAddress && <p className="text-xs text-red-500">{formik.errors.fullAddress as string}</p>}
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-end mb-2">
                            <div>
                                ...
                                <p className="text-xs text-slate-400">Klik peta untuk menyesuaikan titik koordinat.</p>
                            </div>
                            <button
                                type="button"
                        ...
                        <div className={cn(
                            ...
                            hasLocationError ? "border-red-500 ring-2 ring-red-500/20" : "border-slate-200"
                        )}>
                            <MapPicker
                    ...
                    <div className="flex items-start space-x-3 py-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                        <Checkbox id="isPrimary" checked={formik.values.isPrimary} onCheckedChange={(checked) => formik.setFieldValue("isPrimary", checked)} />
                        <div className="grid gap-1.5 leading-none">
                            <Label htmlFor="isPrimary" className="cursor-pointer font-semibold text-slate-700">Jadikan Alamat Utama</Label>
                        </div>
                    </div>
                    ...
                    <div className="pt-2">
                        <Button type="submit" className="w-full h-12 text-base rounded-xl" disabled={isLoading}>
                            {isLoading ? "Menyimpan..." : <><FiSave className="mr-2" /> {isEditMode ? "Simpan Perubahan" : "Simpan Alamat"}</>}
                        </Button>
                        <button type="button" onClick={onClose} className="w-full mt-3 py-3 text-slate-400 font-semibold text-sm hover:text-slate-600 transition" disabled={isLoading}>Batal</button>
                    </div>
                </form>
            </div>
        </div>
